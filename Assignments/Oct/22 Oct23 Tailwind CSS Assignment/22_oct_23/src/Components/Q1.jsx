import React from "react";

const Q1 = () => {
  return (
    <div>
      <div class="flex mx-auto justify-center h-screen items-center p-2">
        <div class="w-96 flex justify-center flex-col text-center">
          <strong class="my-1">Login</strong>
          <h2 class="text-center mb-4">
            Enter your credentials to access your account
          </h2>

          <button class="flex py-2 mb-8  mt-4 border w-full border-green-400 items-center justify-center">
            <img
              class="w-8 border-sky-600"
              src="data:image/png;base64,"
              alt=""
            />
            <div class="w-3"></div>
            <h3>Login with Google</h3>
          </button>
          <div class="flex justify-between items-center mb-6">
            <div class="w-full">
              <hr class="border-sky-600" />
            </div>
            <h3 class="mx-2 font-bold">Or</h3>
            <div class="w-full">
              <hr class="border-sky-600" />
            </div>
          </div>
          <div class="flex flex-col text-left">
            <label for="email" class="mb-1">
              Email Address
            </label>
            <input
              type="text"
              name="email"
              class="border rounded-sm mb-2 h-9 outline-0 pl-2  border-sky-500"
            />
            <label for="password" class="flex justify-between mb-1">
              <span>Password</span>
              <span class="text-blue-700">forgot password?</span>
            </label>
            <input
              type="text"
              name="password"
              class="border rounded-sm h-10 pl-2 outline-0 border-sky-500"
            />
            <button class="py-1 bg-blue-700 my-6">Login</button>
            <span class="mb-4 text-center">
              Don't have an account? Register here
            </span>
          </div>
        </div>
        <div class="hidden w-60 h-screen flex-row-reverse justify-end items-end sm:hidden md:block">
          <img
            class="self-end mt-60 ml-24"
            src="https://img.freepik.com/free-photo/young-happy-woman-with-good-idea-sign-white-shirt-full-portrait_186202-4377.jpg?w=360&t=st=1701585911~exp=1701586511~hmac=82a853f595ca36c4c07189166901d30d5d864b1104426e8076b26cca7bfde38c"
            alt=""
          />
        </div>
      </div>
    </div>
  );
};

export default Q1;
